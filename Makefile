...
local: bin
	...
	cp ../libdriveless/python_bind/app/pydriveless/cpp/libdriveless.so ./libdriveless.so
	rm -rf build

install: bin 
	cd build && sudo make install
	cp build/libfastrrt.so.2.1.0	python_bind/app/pyfastrrt/cpp/libfastrrt.so
