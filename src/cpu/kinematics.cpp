 ...
 double compute_euclidean_2d_dist(const double2 &start, const double2 &end)
{
 ...
 double compute_euclidean_2d_dist(const int2 &start, const int2 &end)
{
    double dx = end.x - start.x;
    double dy = end.y - start.y;
    return sqrt(dx * dx + dy * dy);
 ...
 double compute_path_heading(const double2 p1, const double2 p2)
{
    double dy = p2.y - p1.y;
    double dx = p2.x - p1.x;
    ...
    if (dy >= 0 && dx > 0) // Q1
        ...
        return ANGLE_HEADING_90;
    else if (dx == 0 && dy < 0)
        return -ANGLE_HEADING_90;
    return 0.0;
}
 double compute_path_heading(pose p1, pose p2)
{
    double dy = p2.y - p1.y;
    double dx = p2.x - p1.x;
...
#define MIN_PATH_SIZE 1
 float4 check_kinematic_new_path(int4 *graph, float4 *graphData,
                                                    ...
                                                    float3 ogStart, int2 start, float steeringAngle, float pathSize, float velocity_m_s)
{
 ...
 bool check_kinematic_connection_start_end(int4 *graph, float4 *graphData, float3 *frame, double *physicalParams, int *searchSpaceParams, float *classCost, float3 ogStart, int2 start, int2 end, float velocity_m_s, double &final_heading, double &path_cost)
{
...
bool CudaGraph::checkFeasibleConnection(float3 *og, int2 init, int2 end, int velocity_m_s)
{
 ...
 bool check_graph_connection_with_hermite(
    int4 *graph,
    float4 *graphData,
    float3 *frame,
