...
extern  long computePos(int width, int x, int z);
 ...
 float getFrameCostCpu(float3 *frame, float *classCost, long pos)
{
    int segmentation_class = TO_INT(frame[pos].x);
    return classCost[segmentation_class];
}

 bool isAllAngleFeasible(float3 *frame, long pos)
{
    return TO_INT(frame[pos].z) & 256 > 0;
}

 bool __computeFeasibleForAngle(float3 *frame, int *params, float *classCost, int minDistX, int minDistZ, int x, int z, float angle_radians)
{
 ...
 bool checkStateFeasible(float3 *searchFrame, int *params, float *classCosts, float4 *waypoints, int waypoints_size, int current_pos_waypoints, int minDistX, int minDistZ)
{
    float heading;
