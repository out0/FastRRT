...
TEST(TestGraph, TestPhysicalParams)
{
    ...
    ASSERT_FLOAT_EQ(100/11.1, p[PHYSICAL_PARAM_RATE_W]);
    ASSERT_FLOAT_EQ(11.1/100, p[PHYSICAL_PARAM_INV_RATE_W]);
    ASSERT_FLOAT_EQ(100/11.2, p[PHYSICAL_PARAM_RATE_H]);
    ASSERT_FLOAT_EQ(11.2/100, p[PHYSICAL_PARAM_INV_RATE_H]);
    ASSERT_EQ(angle::deg(21), angle::rad(p[PHYSICAL_PARAM_MAX_STEERING_RAD]));
    ASSERT_EQ(angle::deg(21), angle::deg(p[PHYSICAL_PARAM_MAX_STEERING_DEG]));
    ASSERT_FLOAT_EQ((11.3/2), p[PHYSICAL_PARAM_WHEELBASE]);
}
