...
/// @return
void __check_direct_connection_to_forward_nodes(int4 *graph, float4 *graphData, float3 *frame, float *classCosts, int *searchSpaceParams,
...
{
    std::unique_ptr<float4[]> pathData = std::make_unique<float4[]>(path_size);
    ...
    {
        __check_direct_connection_to_forward_nodes(_graph->getPtr(), _graphData->getPtr(), frame, _classCosts.get(),
                                                   _searchSpaceParams.get(), path.get(), pathData.get(), i, path_size, max_curvature, preProcessCollisionDistance, preProcessDistanceToGoal);
